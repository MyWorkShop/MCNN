...
//	#endif
	...
	void calculate_y(){
		for (int i=0;i<num;i++){
			for (int j=0;j<m;j++){
				...
				printf("D:[%d][%d][%d]=%f\n",i,j,k,d.d[i][j][k]);
			#endif
					y.d[i][j][k]=d.d[i][j][k]+bias.d[i][j][k];
	...
	void calculate_d_w(){
		for (int i=0;i<num;i++){
			for (int j=0;j<m;j++){
				for (int k=0;k<n;k++){
					d_beta.d[i][j][k]+=dleta.d[i][j][k]*(max(i,j,k));
...
//								std::cout<<i<<','<<j<<','<<k<<','<<l_1<<','<<l_2<<','<<l_3<<std::endl;
					...
					dleta.d[i][j][k]=sum/((x_e-x_s)*(y_e-y_s));
				}
			}
		}
	}
