...
public:
	...
	double core(int x_aixs,int y_aixs,int num_source,int num_output);
...
//			std::cout<<"max"<<std::endl; 
			for (int i=0;i<num;i++){
				...
				for (int j=0;j<next_num;j++) {
					sum+=((mat*)((Fully_Connected_Layer*)next_layer)->w)->d[i][j]*((Fully_Connected_Layer*)next_layer)->dleta.d[j];
				} 
...
double Convolutional_Layer::core(int x_aixs,int y_aixs,int num_source,int num_output){
		float sum=0;
		if(s==true){
			for(int i=0;i<a;i++){
				for(int j=0;j<b;j++){
			...
			for (int j=0;j<m;j++){
				for (int k=0;k<n;k++){
					...
					for (int l=0;l<next_num;l++){
						sum+=((Fully_Connected_Layer*)next_layer)->dleta.d[l]*((tube*)((Fully_Connected_Layer*)next_layer)->w)->d[i][j][k][l];
					}
...
//								std::cout<<i<<','<<j<<','<<k<<','<<l_1<<','<<l_2<<','<<l_3<<std::endl;
								sum+=((Convolutional_Layer*)next_layer)->dleta.d[l_1][j-l_2][k-l_3]*((Convolutional_Layer*)next_layer)->w.d[l_1][i][l_2][l_3];
							}
						}
					}
					dleta.d[i][j][k]=sigmoid::df(d.d[i][j][k])*sum;
				}
			}
		}
	}
...
public:
	...
	void train(float *y){
		...
		std::cout<<"FC_9"<<std::endl;
	#endif
		FC_9.change_weight(0.8); 
	#ifdef	_DEBUG_T_
		std::cout<<"FC_8"<<std::endl;
	#endif
		FC_8.change_weight(0.8); 
	#ifdef	_DEBUG_T_
		std::cout<<"FC_7"<<std::endl;
	#endif
		FC_7.change_weight(0.8); 
	#ifdef	_DEBUG_T_
		std::cout<<"MP_6"<<std::endl;
	#endif
		MP_6.change_weight(0.8); 
	#ifdef	_DEBUG_T_
		std::cout<<"C_5"<<std::endl;
	#endif
		C_5.change_weight(0.8); 
	#ifdef	_DEBUG_T_
		std::cout<<"MP_4"<<std::endl;
	#endif
		MP_4.change_weight(0.8); 
	#ifdef	_DEBUG_T_
		std::cout<<"C_3"<<std::endl;
	#endif
		C_3.change_weight(0.8); 
	#ifdef	_DEBUG_T_
		std::cout<<"MP_2"<<std::endl;
	#endif
		MP_2.change_weight(0.8); 
	#ifdef	_DEBUG_T_
		std::cout<<"C_1"<<std::endl;
	#endif
		C_1.change_weight(0.8); 
	} 
};
